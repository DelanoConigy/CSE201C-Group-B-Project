...
public class Rings extends Room {
	...
	public Rings() {
		totalRingScore = 0;
		stamina = 100;
		moves = new Move[] { new Move("Back Lever", 70, "A challenging static hold parallel to the ground."),
	...
	public Move chooseMove() {
		...
		for (int i = 0; i < moves.length; i++) {
			System.out.println((i + 1) + ". " + moves[i].getName() + " (Difficulty: " + moves[i].getDifficulty()
					+ ") [Description: " + moves[i].getDescription() + "]");
		}
	...
	public void performMove(Move move, Player player) {
		...
		System.out.println("You can hold the " + move.getName() + " for a maximum of " + maxHoldTime
				+ " seconds based on your current stamina.");
	...
	/**
	 * Override of the abstract startRoom() method from the Room class. Shows the
	 * Rings rules, and continually allows the player to select and perform moves
	 * until they're out of stamina.
	 */
	@Override
